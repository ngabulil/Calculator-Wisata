...
const HotelCard = forwardRef(
  ({ index, onDelete, data, onChange, onModalClose }, ref) => {
    ...
    const validateAll = () => {
      ...
      if (index != null) {
        if (!data.hotel) newErrors.hotel = "Hotel harus dipilih";
    ...
    const hargaPerKamar = useMemo(() => {
      ...
      if (data.season.startsWith("high")) {
        ...
        return (
          selectedHotelData.seasons.high.filter((s) => s.idRoom === idRoom)[
            index
          ]?.price || 0
        );
      }
      if (data.season.startsWith("peak")) {
        const index = parseInt(data.season.split("-")[1]);
        return (
          selectedHotelData.seasons.peak.filter((s) => s.idRoom === idRoom)[
            index
          ]?.price || 0
        );
      }
    ...
    return (
      <Box ref={wrapperRef} bg="gray.600" p={4} rounded="md">
