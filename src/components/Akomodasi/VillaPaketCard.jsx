...
const VillaCard = forwardRef(
  ({ index, onDelete, data, onChange, onModalClose }, ref) => {
    ...
    const wrapperVillaRef = useRef(null);
    const [errors, setErrors] = useState({});
    const validateAll = () => {
      const newErrors = {};
      if (index != null) {
        if (!data.villa) newErrors.villa = "Villa harus dipilih";
        if (!data.roomType) newErrors.roomType = "Tipe kamar harus dipilih";
        if (!data.season) newErrors.season = "Musim harus dipilih";
      }
    ...
    useImperativeHandle(ref, () => ({
      ...
      scroll: () => {
        wrapperVillaRef.current?.scrollIntoView({
          behavior: "smooth",
          block: "center",
        });
      },
    ...
    return (
      <Box ref={wrapperVillaRef} bg="gray.600" p={4} rounded="md">
        ...
        <HStack spacing={4} mb={3}>
          <Box w="50%">
            ...
            <MainSelectCreatable
              ...
              styles={{
                borderColor: errors.villa && "red.500",
                border: "1px solid",
              }}
              placeholder="Pilih Villa"
              isError={errors.villa}
            />
            {errors.villa && (
